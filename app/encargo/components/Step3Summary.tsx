...
import { CAKE_PRODUCTS, ADDON_PRODUCTS } from "@/lib/mockData";
...
export default function Step3Summary({ onConfirm, onBack }: Props) {
  ...
  const addons = Object.entries(order.selectedAddons)
    .filter(([, qty]) => qty > 0)
    .map(([id, qty]) => ({
      addon: ADDON_PRODUCTS.find((a) => a.id === id)!,
      qty,
    }))
    .filter((a) => a.addon);

  const addonsTotal = addons.reduce(
    (sum, { addon, qty }) => sum + addon.unitPrice * qty,
    0
  );
  const total = (size?.price ?? 0) + addonsTotal;
  ...
  const waURL =
    selectedDate
      ? buildWhatsAppURL({
          selectedDate,
          selectedCategoryId: order.selectedCategoryId ?? "",
          selectedSizeKey: order.selectedSizeKey ?? "",
  ...
  return (
    <div className="max-w-lg">
      ...
      <div className="bg-surface border border-black/5 rounded-2xl p-6 mb-6 space-y-4">
        ...
        <Row label="Torta" value={product?.name ?? "—"} />
        <Row label="Tamaño" value={size ? `${size.label} — ${size.portions}` : "—"} />
        {addons.map(({ addon, qty }) => (
          <Row
            key={addon.id}
            label={addon.name}
            value={`${qty} ${addon.unitLabel} — $${(addon.unitPrice * qty).toLocaleString("es-AR")}`}
          />
        ))}
      ...
      <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-4 mb-6">
        <p className="text-xs text-amber-800 font-body leading-relaxed">
          <strong>Seña del 50%</strong> para confirmar la reserva.
          El saldo restante se abona antes de la entrega.
      ...
      <div className="space-y-3">
        ...
        {/* Mercado Pago — UI only */}
        <button
          onClick={() =>
            alert(
              "En la versión completa del sitio, serías redirigida a Mercado Pago para abonar la seña del 50%."
            )
          }
          className="flex items-center justify-center gap-3 w-full bg-[#FFE600] text-[#333] font-medium py-4 rounded-full hover:opacity-90 transition-opacity active:scale-95 text-sm border border-[#FFE600]"
        >
          <span className="font-bold text-base">MP</span>
        ...
        <button
          ...
          className="w-full bg-teal text-white font-medium py-4 rounded-full transition-all duration-300 hover:bg-teal-d active:scale-95"
        >
          Confirmar pedido →
        </button>
